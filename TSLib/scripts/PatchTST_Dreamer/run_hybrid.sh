#!/bin/bash
# Hybrid Head: Linear skip + Dreamer residual
...
echo "========================================="
echo "  Hybrid Head (seq=96, e=1, GPU 3)"
echo "  Start: $(date)"
echo "========================================="
...
for dataset in ETTh1 ETTh2 ETTm1; do
    for pred_len in 96 192 336 720; do
        echo ""
        echo "[$(date '+%H:%M:%S')] Hybrid ${dataset} seq=${SEQ_LEN} pred=${pred_len}"
        python -u run.py \
            --task_name long_term_forecast --is_training 1 \
            --root_path ./dataset/ETT-small/ --data_path ${dataset}.csv \
            --model_id ${dataset}_${SEQ_LEN}_${pred_len}_hybrid --model PatchTST_Dreamer --data ${dataset} \
            ...
            --d_latent $D_LATENT --slow_interval $SLOW_INTERVAL --head_variant hybrid \
            --dropout $DROPOUT --learning_rate $LR \
            --batch_size $BATCH_SIZE --des 'Hybrid' --itr 1 \
